document.addEventListener('DOMContentLoaded', function () {
    ...
    function loadTasks() {
        ...
        tasks.forEach(function (task) {
            let commentsHTML = task.comments.length > 0 ? '<ul>' : '<p class="text-muted">No hay comentarios</p>';
            task.comments.forEach((comment, index) => {
                commentsHTML += `
                    <li>
                        ${comment.user}: ${comment.description}
                        <button class="btn btn-danger btn-sm ms-2 delete-comment" 
                        data-task-id="${task.id}" 
                        data-comment-id="${comment.id}" 
                        data-comment-index="${index}">Delete</button>
                    </li>
                `;
            });
            commentsHTML += task.comments.length > 0 ? '</ul>' : '';
            ...
            taskCard.innerHTML = `
                <div class="card">
                    ...
                    <div class="card-footer d-flex justify-content-between">
                        <button class="btn btn-success btn-sm add-comment" data-id="${task.id}">Agregar Comentario</button>
        ...
        document.querySelectorAll('.delete-task').forEach(function (btnDelete) {
            //para cada boton, vamos a manajar el evento click, este evento lo va a manejar la funcion handleDeleteTask
            //definida mas abajo.
            btnDelete.addEventListener('click', handleDeleteTask);
        });

        document.querySelectorAll('.add-comment').forEach(function (btnComment) {
            btnComment.addEventListener('click', handleAddComment);
        });
    ...
    document.getElementById('comment-form').addEventListener('submit', function (e) {
        ...
        const task = tasks.find(t => t.id === taskId);
        task.comments.push({ id: ++commentCounter, user, description });
